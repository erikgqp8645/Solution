...
{
	private class KnownName : Name
	{
		private PredefinedName id = PredefinedName.PN_COUNT;

		public override PredefinedName PredefinedName => id;
	...
	private static KnownName[] _predefinedNames = new KnownName[103]
	{
	...
	private static KnownName[] _otherNames = new KnownName[17]
	{
		new KnownName("true"),
		new KnownName("false"),
		new KnownName("null"),
	...
	private void InitKnownNames()
	{
		if (_knownNames == null)
		{
			...
			int i = 0;
			for (int num = _predefinedNames.Length; i < num; i++)
			{
				Name name = _predefinedNames[i];
				nameTable.Add(name);
			}
			KnownName[] otherNames = _otherNames;
			foreach (KnownName name2 in otherNames)
			{
				nameTable.Add(name2);
			}
			Interlocked.CompareExchange(ref _knownNames, nameTable, null);
		}
	}
